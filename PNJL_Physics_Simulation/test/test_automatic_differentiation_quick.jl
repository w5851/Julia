...
include(joinpath(@__DIR__, "..", "src", "core", "automatic_differentiation.jl"))

@testset "AutomaticDifferentiation quick tests" begin
    # 测试 compute_gradient
    vars = [1.2, -0.5]
    cfg = (T = 1.5, mu = [0.3, 0.4])
    omega = (x, cfg) -> x[1]^2 + 3.0 * x[2] + cfg.T * x[1] + sum(cfg.mu)

    grad = AutomaticDifferentiation.compute_gradient(omega, vars, cfg)
    ...
    dTd = AutomaticDifferentiation.compute_temperature_derivative(omegaT, vars, cfg.T, cfg)
    ...
    dOm_dmu = AutomaticDifferentiation.compute_chemical_potential_derivatives(omegaMu, vars, mu, cfg2)
