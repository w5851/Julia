...
@inline function calculate_energy_sum(masses, p_nodes, coefficient, t_nodes, xi)
    ...
    **DEPRECATED**: This function is deprecated. Use vacuum_energy_integral_aniso instead.
    """
    @warn "calculate_energy_sum is deprecated, use vacuum_energy_integral_aniso instead"
    return vacuum_energy_integral_aniso(masses, p_nodes, coefficient, t_nodes, xi)
end
...
@inline function calculate_log_sum_aniso(masses, p_nodes, Phi1, Phi2, mu, T, coefficient, t_nodes, xi)
    ...
    **DEPRECATED**: This function is deprecated. Use omega_thermal_integral_aniso instead.
    """
    @warn "calculate_log_sum_aniso is deprecated, use omega_thermal_integral_aniso instead"
    return omega_thermal_integral_aniso(masses, mu, T, Phi1, Phi2, p_nodes, coefficient, t_nodes, xi)
end
...
function vacuum_energy_integral_aniso(masses::Vector, grid_p, grid_t, xi::Float64=0.0)
    """
    Calculate vacuum energy integral with anisotropy using new integration interface
    
    Args:
        masses: Vector of quark masses
        grid_p: Momentum grid data
        grid_t: Angular grid data  
    ...
    @inbounds for mass_i in masses
        ...
        integrand = function(p, t)
            ...
            return p^2 * E  # p²因子用于球坐标
        end
        
        # 执行2D积分
        contribution = _integrate_2d_aniso(integrand, grid_p, grid_t)
...
function _integrate_2d_aniso(integrand::Function, grid_p, grid_t)
    """
    执行各向异性模型的2D积分计算
    
    Args:
        integrand: 被积函数 f(p, t)
        grid_p: 动量网格 [nodes, weights]
        grid_t: 角度网格 [nodes, weights]
        
    Returns:
        积分结果
    """
    p_nodes, p_weights = grid_p[1], grid_p[2]
    t_nodes, t_weights = grid_t[1], grid_t[2]
    
    result = 0.0
    
    @inbounds for (i, p) in enumerate(p_nodes)
        @inbounds for (j, t) in enumerate(t_nodes)
            try
                func_value = integrand(p, t)
                if isfinite(func_value)
                    weight = p_weights[i] * t_weights[j]
                    result += func_value * weight
                end
            catch e
                @warn "Integration failed at p=$p, t=$t: $e"
            end
        end
    end
